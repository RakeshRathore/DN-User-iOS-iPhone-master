...
+ (Loading*)sharedView {
    ...
    static Loading *sharedView;
    dispatch_once(&once, ^ { sharedView = [[Loading alloc] initWithFrame:[[UIScreen mainScreen] bounds]]; });
    return sharedView;
}


+ (void)setStatus:(NSString *)string {
	[[Loading sharedView] setStatus:string];
}

#pragma mark - Show Methods

+ (void)show {
    [[Loading sharedView] showWithStatus:nil maskType:SVProgressHUDMaskTypeNone networkIndicator:NO];
}

+ (void)showWithStatus:(NSString *)status {
    [[Loading sharedView] showWithStatus:status maskType:SVProgressHUDMaskTypeNone networkIndicator:NO];
}

+ (void)showWithMaskType:(SVProgressHUDMaskType)maskType {
    [[Loading sharedView] showWithStatus:nil maskType:maskType networkIndicator:NO];
}

+ (void)showWithStatus:(NSString*)status maskType:(SVProgressHUDMaskType)maskType {
    [[Loading sharedView] showWithStatus:status maskType:maskType networkIndicator:NO];
}
+ (void)showWithStatus:(NSString*)status maskType:(SVProgressHUDMaskType)maskType Indicator:(BOOL)Value {
    [[Loading sharedView] showWithStatus:status maskType:maskType networkIndicator:Value];
}
#pragma mark - Show then dismiss methods

+ (void)showSuccessWithStatus:(NSString *)string {
    [Loading showImage:[UIImage imageNamed:@"Loading.bundle/success.png"] status:string];
}
...
+ (void)showErrorWithStatus:(NSString *)string {
    [Loading showImage:[UIImage imageNamed:@"Loading.bundle/error.png"] status:string];
}
...
+ (void)showImage:(UIImage *)image status:(NSString *)string {
    [[Loading sharedView] showImage:image status:string duration:1.0];
}
...
+ (void)dismiss {
	[[Loading sharedView] dismiss];
}

+ (void)dismissWithSuccess:(NSString*)string {
	[Loading showSuccessWithStatus:string];
}

+ (void)dismissWithSuccess:(NSString *)string afterDelay:(NSTimeInterval)seconds {
    [[Loading sharedView] showImage:[UIImage imageNamed:@"Loading.bundle/success.png"] status:string duration:seconds];
}

+ (void)dismissWithError:(NSString*)string {
	[Loading showErrorWithStatus:string];
}

+ (void)dismissWithError:(NSString *)string afterDelay:(NSTimeInterval)seconds {
    [[Loading sharedView] showImage:[UIImage imageNamed:@"Loading.bundle/error.png"] status:string duration:seconds];
}
...
+ (BOOL)isVisible {
    return ([Loading sharedView].alpha == 1);
}
